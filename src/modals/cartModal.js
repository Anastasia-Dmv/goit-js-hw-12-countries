...
export const cartModal = () => {
    const markupCart = template(cart.order);
    ...
    const instance = basicLightbox.create(
      ...
      ${
        cart.order.length > 0
          ? `<ul class="orderList">${markupCart}</ul>`
          : `<p>Cart is empty</p>`
      }
      <hr>
      ...
      ${
        cart.order.length > 0
          ? "<button class='orderButton'>Confirm Order✔️</button>"
          : ''
      }
    ...
    const removeDish = e => {
      if (
        ...
        document.querySelector(
          '.summary',
        ).innerHTML = `<b>Total cost: </b>${cart.totalSum}`;
        document.querySelector(
          '.allProducts',
